...
from app.auth.forms import LoginForm, SignUpForm, ResetPasswordForm, ResetPasswordRequestForm, DeleteAccountForm, \
    ChangeEmailRequestForm
from app.auth.email import send_confirmation_email, send_password_reset_email, send_email_address_change_email, \
...
def change_email_request_view(username):
    ...
    if form.validate_on_submit():
        ...
        flash('Check your new email for the confirmation link to change email address.', 'success')
        return redirect(url_for('main.account_settings_view', username=current_user.username))
...
def sign_up_view():
    ...
    if form.validate_on_submit():
        ...
        password1 = form.password1.data
        new_user = UserModel(
            username=username, password_hash=generate_password_hash(password1, method='sha256'), email=email
        )
        try:
            new_user.save()
        except errors.ValidationError:
