...
from app.main.forms import ContactForm, ReportURLForm, UpdateUrlForm, CreateUrlForm
...
def contact_view():
    ...
    if form.validate_on_submit():
        ...
        context = {'name': name, 'sender_email': sender_email, 'subject': subject, 'message': message}
        send_email(subject='[CONTACT] ' + subject,
...
def report_url_view():
    ...
    if form.validate_on_submit():
        ...
        context = {'sender_email': sender_email, 'short': short, 'long': long, 'reason': reason,
                   'description': description}
        send_email(subject=f'[REPORT] {reason} - {short}',
                   recipients=[current_app.config['MAIL_USERNAME']],
