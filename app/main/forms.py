...
from wtforms.validators import URL, Email, InputRequired, Length, ValidationError
...
class ContactForm(FlaskForm):
    name = StringField(
        "Name",
        validators=[Length(max=30, message="Name cannot be longer than 30 characters!")],
    )
    email = StringField(
        "Email address (required)",
        validators=[
            InputRequired("Provide email address!"),
    ...
    message = TextAreaField(
        "Message (required)",
        validators=[
            InputRequired("Provide a message!"),
...
class UrlFormMixin:
    long = StringField(
        "Original URL",
        validators=[InputRequired("Provide URL!"), URL(message="URL not correct!")],
    )
    short = StringField(
        "Short URL",
        validators=[
            InputRequired("Provide short URL!"),
            Length(min=1, max=7, message="Short URL must be between 1 and 7 characters!"),
            SafeCharacters(
                chars=set(string.punctuation),
                message="Short URL cannot have special characters!",
            ),
        ],
    )
    valid_til = DateTimeField(
        "Valid Til",
        format="%Y-%m-%d %H:%M",
        validators=[InputRequired("Provide expiration date!")],
    )
...
class CreateUrlForm(FlaskForm, UrlFormMixin):
    short = StringField(
        "Short URL",
        validators=[
            Length(max=7, message="Short URL must be between 1 and 7 characters!"),
            SafeCharacters(
                chars=set(string.punctuation),
                message="Short URL cannot have special characters!",
            ),
